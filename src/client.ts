// src/client.ts
import { getEventHash, finalizeEvent, generateSecretKey, getPublicKey, Relay, Event, EventTemplate, UnsignedEvent } from 'nostr-tools'
...
// WebRTC Peer setup
function createPeer(initiator: boolean, targetPubkey: string, sessionId: string): RTCPeerConnection {
...
async function publishSignaling(
    ...
    const tags: string[][] = [['p', peer], ['session', sessionId]]
...
export async function startSession(targetPubkey: string) {
    ...
    await publishSignaling(kindOffer, targetPubkey, sessionId, { sdp: btoa(offer.sdp!) })
    log('📨 Offer sent to ' + targetPubkey)
}

// Handle incoming signaling events
async function handleEvent(event: Event) {
    const tags: Record<string, string> = Object.fromEntries(event.tags.map(t => [t[0], t[1]]))
    if (tags.p !== pk) return
    ...
    if (event.kind === kindOffer) {
        ...
        await publishSignaling(kindAnswer, event.pubkey, tags.session, { ref: event.id, sdp: btoa(answer.sdp!) })
        log('📨 Sent answer')
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('start')!.onclick = () => {
        const peerPub = (document.getElementById('peerPub') as HTMLInputElement).value.trim()
